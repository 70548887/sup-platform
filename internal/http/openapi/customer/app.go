...
// custAppResponse API应用返回结构（不含Secret）
type custAppResponse struct {
	ID          uint   `json:"id"`
...
// custAppDBRow 查询api_apps表的中间结构
type custAppDBRow struct {
...
func (custAppDBRow) TableName() string {
	return "api_apps"
}

// AppCreate POST /openapi/customer/App/Create — 创建API应用
...
// AppList GET /openapi/customer/App/List — 查询当前用户的所有API应用
...
// AppDetail GET /openapi/customer/App/Detail — 查看单个应用详情
func (h *Handler) AppDetail(c *gin.Context) {
	...
	response.Success(c, custAppResponse{
		ID:          app.ID,
		AppID:       app.AppId,
		AppName:     app.AppName,
...
// AppRotateKey POST /openapi/customer/App/RotateKey — 轮换密钥
func (h *Handler) AppRotateKey(c *gin.Context) {
	...
	now := time.Now()
	if err := h.DB.Model(&custAppDBRow{}).Where("id = ?", app.ID).Updates(map[string]interface{}{
...
// AppUpdateStatus PATCH /openapi/customer/App/Status — 启用/禁用应用
...
// AppDelete DELETE /openapi/customer/App/Delete — 删除应用
func (h *Handler) AppDelete(c *gin.Context) {
	...
	if err := h.DB.Delete(&custAppDBRow{}, app.ID).Error; err != nil {
